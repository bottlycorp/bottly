...
export const execute: CommandExecute = async(command, user) => {
  ...
  if (!user.isPremium) {
    command.editReply(translate(command.locale, chat.config.exec.notPremiumDownload));
    colors.error(userWithId(command.user) + " tried to download a discussion while not being premium");
    return;
  }

  await command.editReply({ content: translate(command.locale, chat.config.exec.creatingFile), embeds: [], components: [] });
  const file = await createTranscript(channel, { saveImages: true, poweredBy: false /** sorry bro 🙏 */ });
  await command.editReply({ content: translate(command.locale, chat.config.exec.createdFile), embeds: [], files: [file] });
};